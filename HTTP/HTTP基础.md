...
## 6. <a name="Six">鉴权和web安全</a>

当用户登录网站时，服务器验证用户是否拥有访问权限的操作就叫鉴权。接下来将介绍三种鉴权认证机制： session 认证，jwt 认证以及 OAuth2 认证。

#### 6.1 session 认证

##### 6.1.1 session 认证的流程

* 用户第一次进入网站时，服务器会要求注册，会把用户设置的密码加盐后进行散列处理得到hash值，然后把 "用户名 + 盐 + hash值" 存到数据库
* 用户第一次正式登录时，服务器会查询数据库，查到对应的用户名时，将接收到的密码进行加盐后计算hash值，如与存储的一致，确认用户身份，则会创建一个 session对象(存到内存或数据库)，把session对象中的 session_id 通过 Set-Cookie 传给客户端
* 当客户端再发送请求时，会通过请求头中的cookie 属性带上设置的 session_id, 服务器接收到请求后对比自己的 session集合，能找到 session_id 对应的 session 对象，则会把请求的信息返回

##### 6.1.2 如何使session认证更安全

* Set-Cookie 时加上 HttpOnly，这样就可以保证不能通过 `document.cookie` 等js操作访问所在页面的cookie，防范xss攻击；
* 使用 https 对 http报文进行加密

##### 6.1.3 session机制的缺点

* 无法应用所有的场景，只有遵循http协议的浏览器才可以使用session机制，譬如移动端小程序等就无法使用；
* 需要在服务器端存储大量信息，会消耗大量内存或需要操作数据库；当使用分布式服务器时，还需要实现session数据共享，每台服务器都可以读取session。

因此引入jwt鉴权认证机制。

#### 6.2 Json Web Token(jwt) 认证

jwt机制即采用 token 进行鉴权。 其原理是，服务器认证后，生成一个json对象发回给客户端。客户端在后续与服务器通信时，都需要发回这个json对象，服务器则靠这个对象认定用户身份，当然为了保证这个对象不被篡改，服务器在生成此对象时会添加签名，此时服务器就**不用保存任何 session 数据**，服务器无状态，从而比较容易扩展。

##### 6.2.1 jwt认证的流程

* 假设用户A已经注册，此时要登录，服务器在计算密码与存储的一致之后，则生成一个token,返回给客户端。
```js
  // token的生成规则
  str1 = Base64({"alg":"HS256","typ":"JWT"})    //指定签名算法和令牌类型
  str2 = Base64({"username":"A","exp":"xxxx"})  //指定实际传送的数据，包括过期时间等
  str3 = HMACSHA256(str1+"."+str2, secret_key)  //使用服务器私有的密钥对数据进行签名
  token = str1 + "." + str2 + "." + str3        // token是明文的，所以最好进行加密，使用HTTPS进行传输
```
* 客户端收到后存起来(存到Cookie,LocalStorage等地方)，后续发请求时在请求头的 `Authorization`字段中带上这个token，或者是在跨域时放到post请求的数据体里面。

* 若服务器校验 token有效，则返回请求的数据。

##### 6.2.2 缺点
  * 无法废弃某个 token，比如用户注销后，token还处于有效期，此时服务器若不部署额外逻辑，则可能被恶意用户所利用，
  * 需要控制 token 的过期时间，太短则需要用户频繁登录，太长则可能会被盗用
  * token 中含有用户信息，Base64是可逆的，容易被解码获取，需要使用HTTPS协议传输


#### 6.3 OAuth2 认证
OAuth 是**让第三方应用不需要用户名密码来读取用户数据的一个认证过程**。

这里假设**用户U**，需要在**网站A**上进行一些操作，这时需要登录网站A，可以选择注册一个网站A的账户，或者使用**网站S**(认证服务器)的账号登录，此时选择后者，则就开始了OAuth认证。整个流程如下：

* **网站A** 先和 **网站S** 协商，说我以后需要在你这里要一些用户的信息，先登记一下我要用哪些信息，网站S会给网站A返回两个东西：**ClientID** 和 **ClientSecret**，相当于网站A的名片和密钥。

* 此时**用户U**进入了**网站A**，想要进行操作，就说，我要用**网站S**的账户信息登录；
* **网站A**就说， 好的，拿着我的名片(**ClientID**)，去找**网站S**要一个允许凭据；

* **用户U**就对**网站S**说，这是网站A的名片，我要用你的凭据登录网站A，给我一个凭据；
* **网站S**就对**用户U**说，你确定授权吗，他会要你的这些信息，并把网站A想要获取的信息摆出来；
* **用户U**若是选择拒绝，则认证结束，以失败告终；若是选择确定，网站S则会把仅供网站A使用的凭据给用户U，并记录这次协商；

* **用户U**把凭据给**网站A**；

* **网站A**拿着凭据去找**网站S**，网站S根据网站A提供的 ClientID，ClientSecret 以及凭据验证其身份和权限，则给了网站A一个通行令牌(access_token)；
* **网站A**凭着这个令牌则可以获取用户U的信息。

其中，通行令牌是有过期时间的，若是过期了就需要使用 refresh_token 重新签证。

#### 6.4 XSS攻击
XSS(cross-site scripting) ，跨站脚本攻击，是代码注入的一种攻击方式。攻击者通过在目标网站上注入恶意脚本，使之在用户的浏览器上运行， 利用这些恶意脚本，攻击者可以获取用户的敏感信息如 Cookie、 SessionID等，进而危害数据安全。

如恶意用户A在网站上发了一条评论，`<script>alert('xss')</script>` ; 此时用户B去打开这个网页时，在渲染网页的时候则会执行这段恶意脚本，则会弹出"xss"。

可以考虑从以下方面去防范XSS攻击：
  * 后端在返回页面时，将可疑符号变为HTML实体，如 `<` 转变为`&lt;`；
  * 前端要避免拼接HTML，尽量使用`createElement`，`setAttribute`的方法实现，少用innerHTML,尽量使用innerText；
  * 少用 eval



#### 6.5 CSRF 攻击
CSRF（Cross-site request forgery），跨站请求伪造，即攻击者在用户登入页面的情况下，当用户被诱导登入危险网站时，会盗用用户的身份自动向登入的页面发起请求，用户在登录状态下这个请求被服务端接收后会被误以为是用户合法的操作。

服务端可以采用referer验证，http请求头包含referer的字段，用于记录http请求的源地址，可以对比referer的值是否处于自己的白名单或者进行同源验证，但并非万无一失，还可以采用token方案来防范。

采用anti-csrf-token来防范csrf，其原理是在服务器首次响应请求时，生成一个随机token(包含随机字符串以及时间戳)的加密运算的值(如hash，md5)以hidden的形式种入用户的表单页面，同时将token存在服务器的session对象中，当用户再次提交请求时，服务器解密token，对比随机字符串及时间戳，若字符串一致且时间未过期则认为token有效，访问有效。
